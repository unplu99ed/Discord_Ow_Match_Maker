...
const GITHUB_URL = "Git: https://github.com/unplu99ed/Discord_Ow_Match_Maker";
...
const url = (request) => {
    let promise = new Promise(function (resolve, reject) {
        let resultMessage = ""
        if (config.urlMessage && config.urlMessage !== "") {
            resultMessage += config.urlMessage;
            resultMessage += "\n";
        }
        resultMessage += GITHUB_URL;
...
function checkConfigValues() {
    let game_size = config.gameSize;
    if (game_size % 2 != 0) {
        throw "game size have odd number must be even";
    }
