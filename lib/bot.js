"use strict";
const token = "TypeUrToken";
const Discord = require("discord.js");
const queueMngr = require("./QueueMngr");
const queueMngrInstance = queueMngr.getInstance();

const register = (request) => {
    queueMngrInstance.register(request.user, request.data);
    return request.user + " registered successfully."
}
...
const status = (request) => {
    return queueMngrInstance.status();
}
...
module.exports = function () {
    ...
    client.on('message', function (msg) {
        if (msg.content[0] === "!") {
            ...
            let result = factoryMapping[request.key](request);
